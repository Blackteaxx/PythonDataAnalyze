...
namespace WinFormsApp3
{
    ...
    {
        string folderPath = Path.Combine("D:\\", "yunpan");

        private string connectionString = "Data Source=LAPTOP-IQOFIJTE;Initial Catalog=test;User Id=hh;Password=123"; // 数据库连接字符串模板
        private string downloadFolderPath = "D:\\xiazai"; // 下载文件夹路径
        private string uploader = "username1";//更换为登录时的username;
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        // 将文件复制到本地文件夹
                        string localFolderPath = "D:\\yunpan"; // 替换为你的本地文件夹路径
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                if (isSelected)
                {
                    try
                    {
                        ...
                        File.Copy(filePath, downloadFolderPath, true);
