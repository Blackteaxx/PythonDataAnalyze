...
tempfun <- function(over, both, under, rose) {
  ...
  tree.over <-  C5.0(result ~ n_tokens_title + 
                             n_tokens_content + 
    num_hrefs + num_self_hrefs + num_imgs + num_videos + average_token_length + 
    data_channel_is_lifestyle + data_channel_is_entertainment + 
    data_channel_is_bus + data_channel_is_socmed + data_channel_is_tech + 
...
tree.under <-  C5.0(result ~ n_tokens_title + 
                             n_tokens_content + 
    num_hrefs + num_self_hrefs + num_imgs + num_videos + average_token_length + 
    data_channel_is_lifestyle + data_channel_is_entertainment + 
    data_channel_is_bus + data_channel_is_socmed + data_channel_is_tech + 
...
tree.both <-  C5.0(result ~ n_tokens_title + 
                             n_tokens_content + 
    num_hrefs + num_self_hrefs + num_imgs + num_videos + average_token_length + 
    data_channel_is_lifestyle + data_channel_is_entertainment + 
    data_channel_is_bus + data_channel_is_socmed + data_channel_is_tech + 
...
tree.rose <-  C5.0(result ~ n_tokens_title + 
                             n_tokens_content + 
    num_hrefs + num_self_hrefs + num_imgs + num_videos + average_token_length + 
    data_channel_is_lifestyle + data_channel_is_entertainment + 
    data_channel_is_bus + data_channel_is_socmed + data_channel_is_tech + 
...
result ~ timedelta + noun_count + average_token_length + topicNo + n_tokens_content + global_subjectivity + wordRatioIn8000 + All.Possible.Meanings + novel.of.title + global_rate_positive_words + VWordRatio + ContentFleschReadingEase + num_hrefs + NWordRatio + global_sentiment_polarity + global_rate_negative_words + Flesch.Kincaid.Grade.of.Title + num_imgs + num_self_hrefs + data_channel_is_world + n_tokens_title + title_sentiment_polarity + data_channel_is_entertainment + title_subjectivity + is_weekend
...
explainer_rf <- DALEX::explain(rf.binarymodelDown2, label = "rf", data = test, y = test$result)
...
plotPDPPos("global_subjectivity")
plotPDPNeg("global_subjectivity")
...
rf.binarymodelDown2 <- randomForest(result ~ noun_count + topicNo + n_tokens_content + global_subjectivity + wordRatioIn8000 + All.Possible.Meanings + novel.of.title + global_rate_positive_words + VWordRatio + ContentFleschReadingEase + num_hrefs + NWordRatio + global_sentiment_polarity + global_rate_negative_words + Flesch.Kincaid.Grade.of.Title + num_imgs + num_self_hrefs + data_channel_is_world + n_tokens_title + title_sentiment_polarity + data_channel_is_entertainment + title_subjectivity + is_weekend, data = under_sampled4, mtry=10, ntree=300,
                                    importance=T, proximity=T)

# 这个结果会抽风，就算0.68了，可能出不来0.68
getResDown5(rf.binarymodelDown2)

# timedelta : [1] 0.7104191 0.6455479 0.7138011 0.6798977
```
ICE
```{r}
library(ICEbox)
test.df <- select(test, -c("result"))

prob <- predict(rf.binarymodelDown2, newdata = test, type = "prob")
preds <- data.frame(prob)


pred_class <- ifelse(preds$X1 > 0.5, 1, 2)
preds$class <- pred_class
t <- table(preds$class, test$result, dnn = c("预测值","真实值"))
t
getScoresDown5(t)
tableDown5(rf.binarymodelDown2)
getResDown5(rf.binarymodelDown2)
hist(preds$X1)



# ICEbox::ice(rf.binarymodelDown2,test.df, predictor = "Flesch.Kincaid.Grade.of.Title", nlines=100, preProbs = preds)
